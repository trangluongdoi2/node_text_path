...
app.get('/', async (req: Request, res: Response) => {
  ...
  // const url = 'http://192.168.100.25:9000/chromium';
  ...
  // await webSocketStreaming(page as Page).catch(console.error)
...
app.listen(PORT, async () => {
  ...
  async function getChunkedSVGContent2(page: Page, options: any) {
    const size = await page.evaluate((opt) => {
      return new Promise(resolve => {
        ...
        const svgContent = (window as any).corjlFabricCanvas.exportNew('SVG', opt);
  ...
  console.log('START...');
  const res = await getChunkedSVGContent2(page as Page, options);
  const filePath = `hehe-${Date.now()}.svg`
  fs.writeFileSync(filePath, res)
  // const canvasHandler = await page?.evaluateHandle(() => (window as any).corjlFabricCanvas);
  // await canvasHandler?.evaluateHandle(async (canvas: any, options: any) => {
  //   const svgContent = canvas.exportNew('SVG', options);
  //   // console.log(svgContent, '==> svgContent..')
  //   // await fetch('http://localhost:3000/save-file', {
  //   //   method: 'POST',
  //   //   headers: {
  //   //     "Content-Type": "application/json",
  //   //   },
  //   //   body: JSON.stringify({ data: svgContent }),
  //   // });
  ...
  // await page?.exposeFunction('saveToFile', async (content: string) => {
  //   fs.writeFileSync('test.svg', content)
  // });
  // await page?.evaluate(async (opts: any) => {
  //   const svgContent = (window as any).corjlFabricCanvas.exportNew('SVG', opts)
  //   return (window as any).saveToFile(svgContent)
  // }, options);
  console.log('END...');
});