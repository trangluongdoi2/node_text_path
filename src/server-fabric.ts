...
import { prepareWorkingDir, setOutputImagePPI, setOutputImagePPI2 } from './helper/file';
...
app.listen(PORT, async () => {
  // const filePath = path.join(__dirname, './files/input.jpeg');
  const filePath = path.join(__dirname, './files/input.jpeg');
  const buffer = fs.readFileSync(filePath);
  const input = {
    pixelsPerInch: 300,
  }
  const hehe = await setOutputImagePPI(buffer, input);
  // const hehe2 = await setOutputImagePPI2(buffer, input);
  // console.log(hehe, 'hehe...');
  const outputPath = path.join(__dirname, './files/output-imagemagick.jpeg');
  fs.writeFileSync(outputPath, hehe);
})